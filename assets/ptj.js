...
const totalProjects = projectCarousel.children.length;
...
function showImage(projectIdx, imageIdx) {
  ...
  const totalImages = imageSlide.children.length;
  if (imageIdx < 0) imageIdx = totalImages - 1;
  if (imageIdx >= totalImages) imageIdx = 0;
  imageIndexes[projectIdx] = imageIdx;
  imageSlide.style.transform = `translateX(-${imageIdx * 100}%)`;
}

function nextImage(projectIdx) {
